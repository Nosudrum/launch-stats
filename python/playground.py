...
LSPs_dict = {66: 'Soviet Union', 161: 'USAF', 63: 'ROSCOSMOS', 88: 'CASC', 115: 'Arianespace',
             ...
             1016: 'Aus. WRE', 29: 'DLR DE', 106: 'Gen. Dynamics'}
...
for year in range(2022, datetime.now(timezone.utc).year + 1):
    ...
    test1 = plt.hist(F3y_data, bins=np.append(days, max(days) + 1), histtype='step', cumulative=True, stacked=False,
                     linewidth=2, label=F3y_LSPs_Labels, color=colors[0:len(F3y_data)])
    test3 = np.histogram(F3y_data, bins=np.append(np.array(days), max(days) + 1))
