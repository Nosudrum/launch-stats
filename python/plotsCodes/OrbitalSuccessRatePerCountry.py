...
def main(pbar, show=True):
    ...
    for Country in tqdm(
        ...
        for year in Years_list:
            successes = (
                (PastT0s["net"].dt.year == year) & success_mask & country_mask
            ).sum()
            failures = (
                (PastT0s["net"].dt.year == year) & failure_mask & country_mask
            ).sum()
        ...
        axes[0].set(
            ...
            xlim=[min(Years_list) - 0.5, max(Years_list) + 0.5],
