...
from plotsCodes.PlotFunctions import Countries_dict, LSPs_dict, colors, subtitle_html, finish_plotly_figure, datetime, \
    ceil
...
def main():
    ...
    data = Date.join(LSP).join(T0).join(Pad).join(Location).join(PastName).join(
        Outcome).join(Countries)
    ...
    fig = px.histogram(data, x="Date", color="LSP", marginal="rug", hover_data=hover_data, hover_name="name",
    ...
    fig.update_layout(yaxis_title="Total launches per year",
                      ...
                      title_text="Orbital launch attempts per LSP since " + str(
