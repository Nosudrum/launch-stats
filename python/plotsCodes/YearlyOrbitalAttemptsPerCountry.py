...
def main(pbar, show=False):
    ...
    F1_Countries_sorted = (
        F1_Countries["location.country_code"].value_counts().index.tolist()
    )
    F1_Countries_selected = F1_Countries_sorted[0:7]
    F1_Countries[
        ~F1_Countries["location.country_code"].isin(F1_Countries_selected)
    ] = "OTH"
    ...
    for year in tqdm(
        ...
        F1y_Countries[
            ~F1y_Countries["location.country_code"].isin(F1_Countries_selected)
        ] = "OTH"
        ...
        for ii in enumerate(F1_Countries_selected):
            F1y_data_tmp = F1y_T0s[F1y_Countries["location.country_code"] == ii[1]][
                "net"
        ...
        finish_figure(
            F1y, F1y_axes, "yearly/orbitalAttemptsPerCountry/" + str(year), show=show
        )
    F1y_README.close()
    pbar.update()
