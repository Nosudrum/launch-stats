...
def main(show=True):
    print('Creating of orbital launch attempts per LSP since 1957 non-stacked')
    ...
    F3_axes[0].set(ylabel='Launches per year', ylim=[0, 130], xlim=[min(F3_Years), max(F3_Years) + 1],
    ...
    finish_figure(F3, 'OrbitalAttemptsPerLSP', show=show)
