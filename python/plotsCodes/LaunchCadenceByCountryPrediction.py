...
def main(pbar, show=False):
    ...
    README.write(
        f"# Orbital attempts per country for the last 8 years (with {current_year} prediction)\n"
    )
    for Country in tqdm(Countries, desc="Countries", ncols=80, position=1, leave=False):
        README.write(
            f"![Orbital attempts by {Countries_dict_tmp[Country]} in the last 8 years]("
            + Countries_dict_tmp[Country].replace(" ", "_")
            + ".png)\n"
        )
        ...
        for year in range(current_year, current_year - 8, -1):
            year_id += 1
            Country_Past_T0s_yearly = Country_Past_T0s[
                Country_Past_T0s["net"].dt.year == year
            ...
            if year == current_year:
                ...
                Country_Future_T0s_yearly = Country_Future_T0s[
                    Country_Future_T0s["net"].dt.year == year
                ]["net"].dt.dayofyear.to_list()
            else:
            ...
            if Country_Past_T0s_yearly:
                count_past, edges_past = np.histogram(
                    Country_Past_T0s_yearly, bins=Past_bins
                )
                axes[0].step(
                    edges_past[:-1],
                    count_past.cumsum(),
            ...
            if Country_Future_T0s_yearly:
                count_future, edges_future = np.histogram(
                    Country_Future_T0s_yearly, bins=Future_bins
                )
                axes[0].step(
                    edges_future[:-1],
        ...
        axes[0].set(
            ...
            + Countries_dict_tmp[Country]
            + " over the last "
            + str(current_year - int(labels[-1]) + 1)
            + " years",
        )
        finish_figure(
            fig,
            axes,
            "byCountry/launchCadence8yearsPrediction/"
            + Countries_dict_tmp[Country].replace(" ", "_"),
            show=show,
        )
    README.close()
