...
def main(show=True):
    LSPs = (
        PastLSPs[
            PastT0s["net"]
            >= datetime(
                datetime.now(timezone.utc).year - 2, 1, 1, 0, 0, 0, 0, timezone.utc
            )
        ]["id"]
        .value_counts()
        .index.tolist()
    ...
    for LSP in LSPs:
        ...
        print(LSPs_dict[LSP])
        README.write(
            f"![First stages reusability by {LSPs_dict[LSP]}]({LSPs_dict[LSP].replace(' ', '_')}.png)\n"
        )
        ...
        for year in years:
            ...
            stock.append(
                LSP_FirstStageReuseYear["NewRecovered"].sum()
                - LSP_FirstStageReuseYear["ReusedLost"].sum()
            )
        stock = np.cumsum(stock)
        stock = np.append(stock, stock[-1])
        ...
        axes[0].bar(
            years,
            new_not_recovered,
            color=reusability_colors[0],
            label=reusability_labels[0],
            width=1,
            align="edge",
        )
        axes[0].bar(
            years,
            new_recovered,
            bottom=new_not_recovered,
            color=reusability_colors[1],
            label=reusability_labels[1],
            width=1,
            align="edge",
        )
        axes[0].bar(
            years,
            not_new_not_recovered,
            bottom=new_not_recovered + new_recovered,
            color=reusability_colors[2],
            label=reusability_labels[2],
            width=1,
            align="edge",
        )
        axes[0].bar(
            years,
            not_new_recovered,
            bottom=new_not_recovered + new_recovered + not_new_not_recovered,
            color=reusability_colors[3],
            label=reusability_labels[3],
            width=1,
            align="edge",
        )
        axes[0].step(
        ...
        axes[0].set(
            ylabel="Total booster flights",
            xlim=[min(years), max(years) + 1],
            title="Reuse of orbital-class boosters by "
            + LSPs_dict[LSP]
            + " since "
            + str(min(years)),
        )
