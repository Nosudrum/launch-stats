...
def main(pbar, show=False):
    ...
    README.write(
        f"# Orbital attempts per LSP for the last 8 years (with {current_year} prediction)\n"
    )
    for LSP in tqdm(LSPs, desc="LSPs", ncols=80, position=1, leave=False):
        ...
        for year in range(current_year, current_year - 8, -1):
            year_id += 1
            LSP_Past_T0s_yearly = LSP_Past_T0s[LSP_Past_T0s["net"].dt.year == year][
                "net"
            ...
            if year == current_year:
                ...
                LSP_Future_T0s_yearly = LSP_Future_T0s[
                    LSP_Future_T0s["net"].dt.year == year
            ...
            if LSP_Past_T0s_yearly:
                count_past, edges_past = np.histogram(
                    LSP_Past_T0s_yearly, bins=Past_bins
                )
                axes[0].step(
                    edges_past[:-1],
                    count_past.cumsum(),
            ...
            if LSP_Future_T0s_yearly:
                count_future, edges_future = np.histogram(
                    LSP_Future_T0s_yearly, bins=Future_bins
                )
                axes[0].step(
                    edges_future[:-1],
        ...
        finish_figure(
            fig,
            axes,
            "byLSP/launchCadence8yearsPrediction/" + LSPs_dict[LSP].replace(" ", "_"),
            show=show,
        )
    README.close()
    pbar.update()
