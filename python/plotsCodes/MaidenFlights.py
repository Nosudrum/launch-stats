from Processing import PastRocket
...
def main(show=True):
    ...
    data = PastRocket[
        ["configuration.name", "configuration.family", "configuration.maiden_flight"]].copy().rename(
        columns={"configuration.name": "name", "configuration.family": "family",
                 "configuration.maiden_flight": "mf"}).drop_duplicates()
    current_year = datetime.now(timezone.utc).year
    families_found = []
    maiden_flights_newFamily = []
    maiden_flights_oldFamily = []
    for _, row in data.iterrows():
        if row["mf"] is not None:
            mf_year = datetime.strptime(row["mf"], "%Y-%m-%d").year
