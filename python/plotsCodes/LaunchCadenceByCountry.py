...
def main(pbar, show=False):
    ...
    for Country in tqdm(
        F5_Countries, desc="Countries", ncols=80, position=1, leave=False
    ):
        F5_README.write(
            "![Orbital attempts by "
            + F5_Countries_dict[Country]
            + " in the last 8 years]("
            + F5_Countries_dict[Country].replace(" ", "_")
            + ".png)\n"
        )
        ...
        for year in range(
            ...
            year_id += 1
            F5_Country_T0s_yearly = F5_Country_T0s[
                F5_Country_T0s["net"].dt.year == year
