...
def main(pbar, show=True):
    ...
    for Country in F2_Countries_selected:
        F2_data.append(
            PastT0s[F2_Countries["location.country_code"] == Country][
                "net"
    ...
    F2_axes[0].hist(
        F2_data,
        bins=np.append(np.unique(F2_Years), max(F2_Years) + 1),
        histtype="step",
        stacked=False,
        label=F2_Countries_Labels,
        color=colors,
        linewidth=1.5,
    )
    ...
    F2_axes[0].set(
        ...
        xlim=[min(F2_Years), max(F2_Years) + 1],
