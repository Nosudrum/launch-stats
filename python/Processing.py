...
Launches = pd.read_json(
    "data/Launches.json", convert_dates=["net", "window_start", "window_end"]
)
...
FirstStageReuse = pd.DataFrame(
    columns=["NewLost", "NewRecovered", "ReusedLost", "ReusedRecovered"],
        ...
        if FirstStage.iloc[i][j] is not None:
            ...
            if "landing.attempt" in FirstStage.iloc[i][j]:
                if FirstStage.iloc[i][j]["landing.attempt"] is True:
                    if (
                        "landing.type.id" in FirstStage.iloc[i][j]
                        and FirstStage.iloc[i][j]["landing.type.id"] == 3
                    ):
                        # Ocean
                        recovered = False
                        ...
                        if LaunchT0.iloc[i][0] <= datetime.now(timezone.utc):
                            print(
                                f"WARNING : Missing landing outcome for launch {LaunchName.iloc[i][0]}"
                            )
                        continue
                else:
                    recovered = False
                ...
                if LaunchT0.iloc[i][0] <= datetime.now(timezone.utc):
                    print(
                        f"WARNING : Missing landing data for launch {LaunchName.iloc[i][0]}"
                    )
                continue
...
past_filter = (LaunchT0["net"] <= datetime.now(timezone.utc)) & (
    LaunchOrbit["orbit.id"] != 15
)
PastID = LaunchID[past_filter].copy()
...
PastDayOfYear.loc[
    ~PastT0s.net.dt.is_leap_year & (PastT0s.net.dt.dayofyear >= 60), "net"
] = (PastT0s.net.dt.dayofyear + 1)
PastDayOfYear.loc[
    PastT0s.net.dt.is_leap_year | (PastT0s.net.dt.dayofyear < 60), "net"
] = PastT0s.net.dt.dayofyear

future_filter = (LaunchT0["net"] > datetime.now(timezone.utc)) & (
    LaunchOrbit["orbit.id"] != 15
)
...
for astronaut in Astronauts["id"].to_list():
    flights = pd.json_normalize(
        Astronauts[Astronauts["id"] == astronaut].flights.to_list()[0]
    )
    landings = pd.json_normalize(
        Astronauts[Astronauts["id"] == astronaut].landings.to_list()[0]
    )
    ...
    if flights.empty:
        ...
        # Remove Apollo 1
        flights = flights[
            flights.id != "a49d791b-3c7f-4ee9-8917-14596a2a1a25"
        ].reset_index()
    ...
    if (
        AstronautsAgency[Astronauts["id"] == astronaut]["id"].head(1).item() != 1024
    ) and (
        AstronautsType[Astronauts["id"] == astronaut]["id"].head(1).item() != 6
    ):  # Virgin Galactic & Non-Human
    ...
    if len(flights) > 0:
        ...
        Astronauts.loc[
            Astronauts["id"] == astronaut, "first_flight_net"
        ] = flights.iloc[0].net
        Astronauts.loc[Astronauts["id"] == astronaut, "last_flight_net"] = flights.iloc[
            -1
        ].net
    ...
    if len(landings) > 0:
        ...
        if landings.iloc[0].mission_end:
            Astronauts.loc[
                Astronauts["id"] == astronaut, "first_landing_mission_end"
            ] = landings.iloc[0].mission_end
        if landings.iloc[-1].mission_end:
            Astronauts.loc[
                Astronauts["id"] == astronaut, "last_landing_mission_end"
            ] = landings.iloc[-1].mission_end
